...
<body class="body">
    ...
    <main class="main">
        ...
        <div class="container galeria d-flex justify-content-center align-items-center">
            ...
            $sql = "select * from viviendas where id ='".$_REQUEST['id']."'";	
            $resul = $bd->query($sql);	
            
            foreach ($resul as $linea)
            {
                echo '<form class="text-center rounded formulario" action = "GuardarModViviendas.php?id='.$_REQUEST['id'].'&tipo='.$_GET["tipo"].'" method = "POST" enctype="multipart/form-data">';
                    ...
                    echo '<tr><td>Tipo</td>';
                    echo '<td><input class="form-control" name="Tipo" type="text" value="'.$linea["tipo"].'"></td></tr>';
