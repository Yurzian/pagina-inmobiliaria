...
<body class="body">
    ...
    <main class="main">
        ...
        <div class="container galeria d-flex justify-content-center align-items-center">
            ...
            $sql = "select * from pisos where id ='".$_REQUEST['id']."'";	
            $resul = $bd->query($sql);	
            
            foreach ($resul as $linea)
            {
                echo '<form class="text-center rounded formulario" action = "GuardarModPisos.php?id='.$_REQUEST['id'].'" method = "POST" enctype="multipart/form-data">';
                    echo '<table class="table table-warning table-striped table-bordered text-center">'; //abrir la tabla
                    echo '<tr><th colspan="2">Modificar Piso</th></tr>';
